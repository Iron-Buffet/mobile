...
class AddToCalendar extends React.Component {
  ...
  getWorkouts = () => {
    ...
    if (selectedParts.length > 0) {
      $get(
        LINKS.WORKOUT_INDEX + '?filter[parts]=' + selectedParts.join(','),
      ).then(res => {
  ...
  renderSwitchers = () => {
    return PARTS.map(p => {
      const val =
        this.state.selectedParts.findIndex(part => part === p.id) !== -1;
      return (
    ...
    return (
      <Wrap>
        ...
        <Block flex>
          {workouts.length > 0 && (
            <Block flex>
              <RNPickerSelect
                ...
                items={workouts}
                placeholder={{label: 'Select Workout'}}
                style={{
          ...
          {emptyWorkouts && (
            <Text style={styles.text}>You have no workouts for this part</Text>
          )}
        </Block>
      </Wrap>
    );
